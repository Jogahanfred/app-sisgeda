...
async function renderizarLista(nuPeriodo) {
	...
	try {
		const lstAlumnos = await fetchListarAlumnos(nuPeriodo);
		allcandidateList = lstAlumnos;
		if (lstAlumnos.length > 0) {
			renderizarCardAlumnos(lstAlumnos, currentPage);
			paginationEvents();
		...
		console.error('Error al mostrar los alumnos:', error);
	}
}
...
function renderizarCardAlumnos(datas, page) {
	...
	function clickPage() {
		document.addEventListener('click', function(e) {
			if (e.target.nodeName == "A" && e.target.classList.contains("clickPageNumber")) {
				...
				renderizarCardAlumnos(allcandidateList, currentPage);
			}
		});
	};
...
document.getElementById("addBtn").addEventListener('click', async function(event) {
	document.getElementById("mdTitulo").innerHTML = "Matricular Alumno";
	document.getElementById("mdAlumno").querySelector(".modal-footer").style.display = "block";
	document.getElementById("add-btn").innerHTML = "Matricular";

	var modal = new bootstrap.Modal(document.getElementById('mdAlumno'));
	modal.show();
...
Array.prototype.slice.call(forms).forEach(function(form) {
	form.addEventListener('submit', async function(event) {
		...
		if (!form.checkValidity()) {
			...
			try {
				const response = await fetchMatricularAlumno(alumno);
...
searchElementList.addEventListener("keyup", function() {
	...
	renderizarCardAlumnos(filterData, currentPage);
});

document.getElementById("mdAlumno").addEventListener("hidden.bs.modal", function() {
	limpiarCampos();
});

async function fetchMatricularAlumno(alumno) {
	try {
		const configHttp = {
			method: 'POST',
			headers: {
		...
		if (!response.ok) {
			const message = `"No se ha podido insertar el alumno: ${response.status}`;
...
async function fetchListarAlumnos(nuPeriodo) {
	try {
		const response = await fetch(`/miembros/listarAlumnos?nuPeriodo=${nuPeriodo}`);
		if (!response.ok) {
			throw new Error(`No se ha podido resolver la lista de alumnos: ${response.status}`);
		}
