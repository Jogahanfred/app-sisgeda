...
async function renderizarLista() {
	...
	try {
		const lstFases = await fetchListarFasesACalificarPorPeriodo();
		
		allcandidateList = lstFases;
		if (lstFases.length > 0) {
			renderizarCardFases(lstFases, currentPage);
			paginationEvents();
...
async function renderizarCardFases(datas, page) {
	...
	for (let i = (page - 1) * itemsPerPage; i < (page * itemsPerPage) && i < datas.length; i++) {
		if (datas[i]) {
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                        	<div class="flex-grow-1 overflow-hidden">
								<p class="text-uppercase fw-medium text-muted text-truncate mb-0"> Sub Fases: ${datas[i].nuTotalSubFase}</p>
							</div>
						...
						<div class="d-flex align-items-end justify-content-between mt-4">
							<div>
								...
								<a href="" class="text-decoration-underline">Ir a la Fase</a>
							</div>
							<div class="avatar-sm flex-shrink-0">
								<span class="avatar-title ${colorText} rounded fs-3">
									${datas[i].noNombre.slice(0,2).toUpperCase()}
								</span>
							</div>
						</div>
			...
			card.addEventListener("click", async function() {
				const idFase = datas[i].idFase;
				try {
					const response = await fetchVerSubFasesInscritos(idFase);
					if (response) {
	...
	function clickPage() {
		document.addEventListener('click', function(e) {
			if (e.target.nodeName == "A" && e.target.classList.contains("clickPageNumber")) {
				...
				renderizarCardFases(allcandidateList, currentPage);
			}
		});
	};
...
const searchElementList = document.getElementById("buscarFase");
searchElementList.addEventListener("keyup", function() {
	...
	renderizarCardFases(filterData, currentPage);
});

async function fetchVerSubFasesInscritos(idFase) {
	try {
		const configHttp = {
			method: 'POST',
			headers: {
		...
		const response = await fetch(`/redireccion/redirectCtrlSubFasesInscritas`, configHttp);
...
async function fetchListarFasesACalificarPorPeriodo(){
	try {
		const response = await fetch(`/fases/listarFasesACalificarPorPeriodo`);
		if (!response.ok) {
