...
async function renderizarCardMisiones(datas, page) {
	...
	for (let i = (page - 1) * itemsPerPageMain; i < (page * itemsPerPageMain) && i < datas.length; i++) {
		if (datas[i]) {
			...
			card.innerHTML = `
                <div class="card card-animate  ribbon-box right border shadow-none overflow-hidden" style="cursor: pointer;">
                    <div class="card-body">
                        ...
                        <div class="d-flex align-items-center">
                        	<div class="flex-grow-1 overflow-hidden">
								<h6 class="text-uppercase fw-medium text-muted text-truncate mb-0"> 
									${datas[i].txNombreTipoMision}
									<p class="text-success mb-3"><i class="ri-arrow-right-up-line fs-13 align-middle"></i> ${datas[i].flChequeo === 1 ? 'Vuelo chequeo' : 'Vuelo rutinario'}</p>
								</h6>
							</div> 
						</div>
						...
						<div class="d-flex align-items-end justify-content-between mt-2">
							<div>
								<h3 class="ff-secondary fw-bold mb-4"><span>${datas[i].coCodigo}</span></h3>
								<div class="d-flex flex-wrap gap-2">
									...
									<div data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="${datas[i].qtNota == null ? 'Aún no se ha calificado': 'Esta es su calificación de la misión'}">
										<input type="radio" class="btn-check" name="productsize-radio" id="productsize-radio2">
										<label class="btn btn-soft-primary avatar-xs rounded-circle p-0 d-flex justify-content-center align-items-center" for="productsize-radio2">
											${datas[i].qtNota == null ? '<i class="ri-file-reduce-line"></i>': datas[i].qtNota}
										</label>
									</div>
									<div data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="06 Items Available">
										...
										<label class="btn btn-soft-primary avatar-xs rounded-circle p-0 d-flex justify-content-center align-items-center" for="productsize-radio3">
											R
										</label>
									</div>
									...
									<div data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Out of Stock">
										...
										<label class="btn btn-soft-primary avatar-xs rounded-circle p-0 d-flex justify-content-center align-items-center" for="productsize-radio4">
											<i class="ri-arrow-right-up-line fs-13 align-middle"></i>
										</label>
									</div>
								</div>
			...
			card.addEventListener("click", async function() {
				...
				try {
					const response = await fetchVisualizarMision(idCalificacion);
					if (response) {
...
async function fetchVisualizarMision(idCalificacion) {
	try {
		const configHttp = {
			method: 'POST',
			headers: {
		...
		const response = await fetch(`/redireccion/redirectCtrlMisionAVisualizar`, configHttp);
