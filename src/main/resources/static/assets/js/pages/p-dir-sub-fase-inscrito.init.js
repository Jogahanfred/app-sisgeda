...
async function renderizarLista() {
	...
	try {
		const lstSubFases = await fetchListarSubFasesACalificarPorPeriodo();
		
		allcandidateList = lstSubFases;
		if (lstSubFases.length > 0) {
			renderizarCardSubFases(lstSubFases, currentPage);
			paginationEvents();
...
async function renderizarCardSubFases(datas, page) {
	...
	for (let i = (page - 1) * itemsPerPage; i < (page * itemsPerPage) && i < datas.length; i++) {
		if (datas[i]) {
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                        	<div class="flex-grow-1 overflow-hidden">
								<p class="text-uppercase fw-medium text-muted text-truncate mb-0"> Misiones: ${datas[i].nuTotalMision}</p>
							</div>
							
							<div class="flex-shrink-0">
								<h5 class="text-success fs-14 mb-0">
								<i class="ri-arrow-right-up-line fs-13 align-middle"></i> Entrenamiento: ${datas[i].nuTotalHora} Hrs
							    </h5>
							</div>
						</div>
						...
						<div class="d-flex align-items-end justify-content-between mt-4">
							<div>
								...
								<a href="" class="text-decoration-underline">Ir a la Sub Fase</a>
							</div>
							<div class="avatar-sm flex-shrink-0">
								<span class="avatar-title ${colorText} rounded fs-3">
									${datas[i].coCodigo.slice(0,2).toUpperCase()}
								</span>
							</div>
						</div>
			...
			card.addEventListener("click", async function() {
				const idSubFase = datas[i].idSubFase; 
				try {
					const response = await fetchVerMisionesInscritos(idSubFase);
					if (response) {
	...
	function clickPage() {
		document.addEventListener('click', function(e) {
			if (e.target.nodeName == "A" && e.target.classList.contains("clickPageNumber")) {
				...
				renderizarCardSubFases(allcandidateList, currentPage);
			}
		});
	};
...
const searchElementList = document.getElementById("buscarSubFase");
searchElementList.addEventListener("keyup", function() {
	...
	renderizarCardSubFases(filterData, currentPage);
});

async function fetchVerMisionesInscritos(idSubFase) {
	try {
		const configHttp = {
			method: 'POST',
			headers: {
		...
		const response = await fetch(`/redireccion/redirectCtrlMisionesInscritas`, configHttp);
...
async function fetchListarSubFasesACalificarPorPeriodo(){
	try {
		const response = await fetch(`/subFases/listarSubFasesACalificarPorPeriodo`);
		if (!response.ok) {
